...
for c in country:
    ...
    for y in year:
        ...
        data.append([c, y, recall_0, recall_1, accuracy])
        df_fit = pd.DataFrame(data,columns=('country', 'year', "recall_0", "recall_1", 'accuracy'))
        df_fit.sort_values(by=['country',"year"], inplace=True)
    
# GRAPH
df_fit_long = pd.melt(df_fit, id_vars=["country","year"])
df_graph = df_fit_long
...
g.savefig(os.path.join(graphs_path,"graph_best.pdf"), bbox_inches='tight')
